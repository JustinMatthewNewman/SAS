...
proc sgplot
data=library.subset;
hbar REGION_GERMANY / stat=mean limits=both alpha=.1; run;
...
data library.subset2; set library.subset2;
if ECON_SIT=8 then delete; run;
if COVID_COUNTRY=8 then delete; run;
...
proc sgplot data=library.subset2;
vbar RESPECT_US / group=REGION_GERMANY barwidth=0.7 groupdisplay=cluster;
run;
...
proc gchart data=library.subset2;
  pie AGE / value=none slice=inside percent=inside;
run;
